...
if __name__ == "__main__":
    ...
    for i in range(num_chunks):
        req_start = time.time()
        send_at_10mbps(chunk_size)
        duration = (time.time() - req_start) * 1000
        print(f"Chunk {i + 1}: {duration:6.2f}ms")
    total = time.time() - start
    print(f"Total: {total:.2f}s\n")

    print("Rate values can be provided as strings (for example: '1mbps', '10mbps').")
    print("Run with the interceptor (LD_PRELOAD) for socket-level effects.")
    print()
