...
if [ $# -eq 0 ]; then
    ...
    echo "  1_http_requests.py   - HTTP with requests library"
    echo "  2_http_async.py      - Async HTTP with aiohttp"
    echo "  3_tcp_client.py     - TCP socket client"
    echo "  4_udp_client.py     - UDP socket client"
    echo "  5_rate_limit.py     - Rate limiting demo"
    echo "  6_multi_protocol.py - Multiple protocols combined"
    echo "  7_latency_jitter.py - Latency and jitter injection"
    echo "  8_bandwidth_throttle.py - Bandwidth throttling"
    echo "  9_network_timeout.py   - Network latency injection"
    ...
    echo "  3_tcp_client.py     - requires TCP echo server on port 9000"
    echo "  4_udp_client.py     - requires UDP echo server on port 9001"
    echo "  13_end_to_end_scenarios.py - TCP 9000, UDP 9001, HTTP 8000"
    echo ""
    echo "Example: $0 1_http_requests.py"
    exit 1
fi
